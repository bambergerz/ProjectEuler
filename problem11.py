def greatest_mult(grid):
	...
	for b in range(17):
		...
		while c < 17:
			#print 'c is ' +  str(c)
			#print 'b is ' + str(b)
			if calc_diag_up_down(grid, b, c) > up_down_diag_val:
				up_down_diag_val = calc_diag_up_down(grid, b, c)
			if calc_hori_up_down(grid, b, c) > up_down_diag_val:
				up_down_diag_val = calc_hori_up_down(grid, b, c)
			if calc_vert_up_down(grid, b, c) > up_down_diag_val:
				up_down_diag_val = calc_vert_up_down(grid, b, c)
			if calc_diag_down_up(grid, 19-b, c) > up_down_diag_val:
				up_down_diag_val = calc_diag_down_up(grid, 19-b, c)
			c+=1
	return up_down_diag_val
	
def calc_diag_up_down(grid, x, y):
	total = 1
	#Back end at (a,b) and front end at (x,y) 
	for z in range(4):
		#print 'multiplying position ' + str(x+z) + ' by ' + str(y+z) + ' with value ' + str(grid[x+z][y+z])
		total *= grid[x +z][y + z]
		#print 'total is ' + str(total)
	return total

def calc_diag_down_up(grid, x, y):
	total = 1
	#Back end at (a,b) and front end at (x,y) 
	for z in range(4):
		#print 'multiplying position ' + str(x+z) + ' by ' + str(y+z) + ' with value ' + str(grid[x+z][y+z])
...
def calc_hori_up_down(grid, x, y):
	total = 1
	#Back end at (a,b) and front end at (x,y) 
	for z in range(4):
		#print 'multiplying position ' + str(x+z) + ' by ' + str(y+z) + ' with value ' + str(grid[x+z][y+z])
...
def calc_vert_up_down(grid, x, y):
	total = 1
	#Back end at (a,b) and front end at (x,y) 
	for z in range(4):
		#print 'multiplying position ' + str(x+z) + ' by ' + str(y+z) + ' with value ' + str(grid[x+z][y+z])
...
row_8 = [67, 26, 20, 68, 02, 62, 12, 20, 95, 63, 94, 39, 63, 8, 40, 91, 66, 49, 94, 21]
row_9 = [24, 55, 58, 5, 66, 73, 99, 26, 97, 17, 78, 78, 96, 83, 14, 88, 34, 89, 63, 72]
row_10 = [21, 36, 23, 9, 75, 0, 76, 44, 20, 45, 35, 14, 00, 61, 33, 97, 34, 31, 33, 95]
...
row_17 = [04, 42, 16, 73, 38, 25, 39, 11, 24, 94, 72, 18, 8, 46, 29, 32, 40, 62, 76, 36]
